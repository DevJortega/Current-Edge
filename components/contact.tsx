...
export function Contact({ isDark }: ContactProps) {
  ...
  return (
    <section
      id="contacto"
      className={`py-12 sm:py-16 md:py-20 px-4 sm:px-6 lg:px-8 ${isDark ? "bg-black" : "bg-white"}`}
    >
      <div className="container mx-auto max-w-6xl">
        <div className="text-center space-y-3 sm:space-y-4 mb-12 sm:mb-16">
          <h2
            className={`font-bold text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-sans ${isDark ? "text-white" : "text-gray-900"}`}
          >
            Conectemos
          </h2>
          <p
            className={`max-w-3xl mx-auto text-base sm:text-lg md:text-xl lg:text-2xl font-sans px-2 ${isDark ? "text-gray-300" : "text-gray-600"}`}
          >
            ...
            <CardContent className="space-y-3 sm:space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div>
                  <label
                    htmlFor="name"
                    className={`block text-xs sm:text-sm font-medium mb-1 sm:mb-2 ${isDark ? "text-gray-300" : "text-gray-700"}`}
                  >
                    Nombre
                  </label>
              ...
              <CardContent className="space-y-3 sm:space-y-4">
                {contactInfo.map((info, index) => (
                  <div key={index} className="flex items-center space-x-3">
                    <div
                      className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${isDark ? "bg-blue-900/30 text-blue-400" : "bg-blue-100 text-blue-600"}`}
                    >
                      <info.icon />
                    </div>
                    <div>
                      <p className={`font-medium text-xs sm:text-sm ${isDark ? "text-white" : "text-gray-900"}`}>
                        {info.label}
                      </p>
                      <p className={`text-xs sm:text-sm ${isDark ? "text-gray-300" : "text-gray-600"}`}>{info.value}</p>
                    </div>
                  </div>
                ))}
              </CardContent>
                ...
                <div className="flex justify-center space-x-3 sm:space-x-4">
                  {socialLinks.map((social, index) => (
                    <button
                      ...
                      className={`
                        group relative p-2 sm:p-3 rounded-lg transition-all duration-300 transform hover:scale-110 shadow-md hover:shadow-lg
                        ${
                          social.gradient
                    ...
                    >
                      ...
                      <div
                        className={`
                        absolute -top-10 left-1/2 transform -translate-x-1/2 px-2 py-1 rounded text-xs font-medium whitespace-nowrap
